...
import com.zigbee.webSocket.ZigbeeWebSocket;
...
public class ZigbeeDataController {
    ...
    public ServerResponse<ZigbeeReceiveData> addZigbeeData(ZigbeeReceiveData zigbeeReceiveData) {
        ...
        // 调用websocket
        ServerResponse serverResponse = iZigbeeDataService.addZigbeeData(zigbeeReceiveData);
        if (ZigbeeWebSocket.getZigbeeWebSocketSet().size() > 0 && serverResponse.getStatus() == ResponseCode.SUCCESS.getCode()) {
            try {
                // 把最新的消息传给客户端
                ZigbeeWebSocket.bordercastMessage(gson.toJson(iZigbeeDataService.getLastZigbeeData()));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
